...
- **AI assistant (hints + commentary)** — a single optional OpenAI-compatible endpoint (its own, independent of the sides) powers two opt-in extras: 💡 **hints** (a human player can ask for a move suggestion, 3 per game, highlighted on the board and never auto-played) and 🗣 **AI commentary** (an entertaining one-line remark after each move, shown under the board and spoken aloud if Speak moves is on).
