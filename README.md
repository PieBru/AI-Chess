...
**Cloud is supported — never required.** LLM-AI is built around a local or self-hosted OpenAI-compatible server: that is the intended, fully-offline path and the spirit of this project. But running your own inference server takes hardware and setup that not everyone has, so the app also works with any cloud-hosted OpenAI-compatible endpoint — and as of today, several have been verified compatible with the app's requirements. Their use is **entirely optional**: the app never requires a cloud account, never collects telemetry, and keeps your key only in your own browser. If you can self-host, do; if you can't, the cloud path is there so the door stays open for you.
