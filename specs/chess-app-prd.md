...
| ~~Spectator reactions~~ (Appendix B #8) | PRD §6 | **Shipped (2026-07-05), upgraded to sampled same day:** off-by-default "Spectator reactions" toggle, persisted. Real crowd reactions under the **Pixabay License** (no attribution, GPL-3.0-compatible): `best`→ applause, `blunder`→ boo, `mistake`→ disappointment, `inaccuracy`→ shocked, checkmate→ cheer. Embedded as base64 in `#reaction-clips-src` (~171 KB raw / ~228 KB base64, 5 clips). Earlier synthesized Web Audio version (noise-burst "claps") sounded like static — replaced. | n/a |
