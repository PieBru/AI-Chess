...
- **Request:** `POST {apiBase}/chat/completions` with `Authorization: Bearer {apiKey}` when a key is supplied; body is `{ model, messages }` plus `reasoning_effort` **only when the side's Reasoning level is set (FR-9.8)**. No temperature/top_p/etc. is ever sent — reasoning models (o-series, R1, Gemini/Claude-thinking) and some third-party servers 400 on unknown non-reasoning params, which would make the model fail every move and poison eval data (cf. LLM-Chess, Apr 2026). The system message constrains the model to reply with exactly one legal UCI move; the user message supplies the FEN, the side to move, recent SAN history, and the full legal UCI move list (constrained choice — the main reliability lever).
- **Parse / retry:** extract the first UCI token from `choices[0].message.content`, preferring an exact legal-move token. A non-matching reply triggers up to one corrective retry: the model's bad reply is appended as an `assistant` message and a `user` message says "that wasn't legal, pick from: …". After a failed retry the function throws. Native reasoning blocks that thinking models inline in the reply (`<thought>`…`</thought>` for Gemini/Gemma, `<think>`…`</think>` for DeepSeek-R1/o1-style) are stripped from the content inside `callLLM` before it returns, so the model's private deliberation never pollutes the move pick (last-wins), the hint's reason line (first-line), or the commentary text — every consumer sees only the actual answer.
...
- **Reasoning level, graceful degradation (FR-9.8):** `reasoning_effort` is model-specific, so it is sent **opportunistically with no capability probe**: if the endpoint returns HTTP 400 while it was sent, the side marks it unsupported for the rest of the game (`sideConfig._reasoningBad`) and retries that move **without** it (`continue` in the retry loop — at most one wasted call per game). So an unsupported knob degrades silently rather than breaking the constrained-choice contract. The level is persisted with the side config and shown in match/tournament side labels (`model [persona, reasoning:high]`).
...
- **Audio subsystem** (spec FR-5.6): all audio is `HTMLAudioElement`-based (no `AudioContext`). Clips are **sampled, embedded as base64 data URIs** in two inline blocks — `#sound-clips-src` (CC0 piece sounds: move/capture/check/game-end, ~31 KB raw) and `#reaction-clips-src` (Pixabay-License crowd reactions: applause/boo/disappointment/shocked/cheer, ~171 KB raw). Two independent off-by-default persisted prefs gate it: `sfx` (piece sounds, any game) and `reactions` (crowd reactions, AI-vs-AI only). Piece sounds fire on move/capture/check/game-end. **Crowd-reaction gating (non-trivial):** a reaction fires only on a *capture* whose **drama score** beats a **ply-decaying linear threshold** — `drama = capturedValue + evalSwingPawns + qualityPremium`, threshold `= base + k·ply` (starts high, decays so the crowd is quiet early and invested by the endgame). Map: `best`→applause, `blunder`→boo, `mistake`→disappointment, `inaccuracy`→shocked, checkmate→cheer. Reaction clips fade out over their last 400 ms. (Earlier synthesized Web-Audio "claps" were replaced with sampled clips after playtest feedback; no `AudioContext` remains.)
- **Hint coach** (spec FR-9.9): `hintCoach` (apiBase/key/model) is persisted in prefs exactly like `ttsConfig`. A human's 💡 Hint button — shown only on a human's turn with a configured coach and `hintsLeft > 0` — calls the coach via the shared `callLLM`, parses the move from the **first reply line only** (the reason on later lines may name other moves, and `extractUciMove` is last-wins, so whole-content extraction would mis-pick), re-validates it, and renders the suggestion as a green `.hint` ring on the from/to squares (cleared on the next move). Because `callLLM` strips `<thought>`/`<think>` reasoning blocks first, the first line is the model's committed move, not its deliberation. `hintsLeft` (3) is game state, reset on new game / take-over; a failed/illegal reply alerts the user and burns no hint.
- **AI commentary** (spec FR-9.10): reuses the same AI-assistant endpoint. A persisted toggle fires `maybeCommentate` from `narrateMove` after each move — a **single fetch (no retry loop)** generating one entertaining line. A `commentaryGen` token (mirroring `speakGen` in `speakText`) drops any comment superseded by a newer move, so fast AI-vs-AI games skip stale commentary instead of queueing it. The comment is appended as a 2nd toaster line and spoken via `speakText` when Speak moves is on (then the factual narration is shown but not spoken). Best-effort: any failure silently drops that move's comment; the loop is never blocked.
- **In-session tally** (spec FR-9.11): a `sessionTally` object ({games, whiteWins, blackWins, draws}) is incremented from `finishGame` for single games only (`!seriesActive()`), keyed off the result string (`1-0`/`0-1`/`1/2-1/2`; `*` skipped). Rendered as a compact panel under the board with a reset link. In-memory only (no persist) — a running record across New game / Rematch for a spectator or human.
- **AI move time cap** (spec FR-9.12): `requestAIMove` wraps the controller call in `Promise.race` against a `setTimeout` when `aiMoveTimeoutSec > 0`. The losing race throws `move timed out after Ns`, which the caller's existing per-controller catch handles — an LLM falls back to a local Normal move (FR-9.4); an engine timeout ends the game (pathological; engines are already bounded). Applies to AI moves only; humans are clock-governed. The setting is a persisted setup-screen selector (Off/15/30/60/120s). **Cancellation:** on timeout an `AbortController` aborts `callLLM`'s in-flight fetch + retry loop (the signal threads `requestAIMove`→`requestLLMMove`→`callLLM`→`fetch`, and an aborted signal is terminal in `callLLM`'s catch — never retried), so a flaky endpoint can't leave orphaned retries running for the 5-min deadline in the background; the post-abort rejection is swallowed (`.catch(()=>{})`) so it never surfaces as unhandled. No-op when the cap is Off (signal is `undefined`).
- **Interface language** (spec FR-9.13): a single global `uiLang` ('en' default | 'it'), set via a 🌐 selector on the setup page and persisted. `applyLang()` runs on load and, for 'it', walks all text nodes (skipping `<script>`/`<style>`) swapping any whose normalized content is a key in the `IT_TEXT` dictionary (the English DOM is canonical; Italian overlays it — no per-element `data-i18n` attrs, and the repeated White/Black/Human/Difficulty strings are covered once). A change saves the pref and reloads. Dynamic strings use `t()`/maps (`describeMove` → `pieceName`/IT quality words; status line, thinking indicator, Pause/Resume button via `pauseLabel`, game-over reasons via `reasonText`). Tier B boundary: tooltips, long hint paragraphs, toasts, and generated result tables stay English.
...
**Move limit (FR-1.4):** also checked at the loop top (after `gameStatus`): `capPly = seriesActive() ? min(moveLimitPly, TOURNAMENT_MAX_PLY) : moveLimitPly`, where `moveLimitPly = moveLimitPref === 'off' ? Infinity : +moveLimitPref * 2`. Reaching `capPly` ends the game as `'1/2-1/2', reason:'move limit (N half-moves) reached'`. Single games use the user's setting (or no cap if Off); series games clamp it to the 200-half-move safety net. `moveLimitPref` is a persisted prefs field alongside `timeControl` (§9).
...
**Series dispatch (FR-9.6 / FR-9.7):** the auto-play "series" modes (Tournament gauntlet, Match) share the game loop. At most one of `tournament` / `match` is active; the loop's series hooks (`seriesActive`, `seriesRecordResult`, `seriesAdvance`, `seriesRenderResults`, `seriesRecordMove`, `seriesRenderProgress`) route to whichever is running, so the loop body stays single-branch. Both modes: colors alternate each game, 200-ply draw cap, spectator pacing + Pause/Stop, timestamped results file; no chess clock in Tournament (always has an LLM side); in Match the clock is a free choice (default Off, honored even with an LLM side). Tournament adds the adaptive level sweep + a Stockfish-`UCI_Elo`-anchored MLE **absolute**-Elo estimate (and scores only its single LLM side); Match adds a fixed best-of-N between the two configured sides with a **relative** Elo difference (logistic from score) + LOS (Bayesian, decisive games), scoring **both** sides' move quality so a head-to-head is fair regardless of controller type; when exactly one side is a Stockfish level with a known UCI_Elo (6–10) it additionally runs the gauntlet's MLE for an **absolute** Elo on the other side (same pool — directly comparable), synthesized from the aggregate W/draw/L against that fixed anchor.
